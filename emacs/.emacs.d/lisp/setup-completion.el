...
;; Once the popup is open, a documentation tooltip appears after a short pause, and your chosen candidates are remembered and reordered by frequency across Emacs restarts 
(use-package corfu
  ;; :ensure t
  :init
...
(use-package cape
  ...
  :init
  (add-hook 'completion-at-point-functions #'cape-file 50)
  (when (fboundp 'yasnippet-capf)
    (add-hook 'completion-at-point-functions #'yasnippet-capf 20))
  (add-hook 'completion-at-point-functions #'cape-abbrev 60)
  :config
  ;; This needs cape to be loaded, so it must live in :config, not :init.
  (add-hook 'completion-at-point-functions (cape-capf-prefix-length #'cape-dabbrev 2) 90)
...
(use-package tempel
 ...
 ;; Needs tempel loaded, so it must live in :config, not :init.
